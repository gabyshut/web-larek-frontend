import { IApiClient, IBasket, ICardData, ICardItem, IUserOrderData, TProductId } from "../types";
...
export class UserOrderModel extends EventEmitter{
    ...
    setField<K extends keyof IUserOrderData>(field: K, value: IUserOrderData[K]): void {
    ...
    validate(field: keyof IUserOrderData): boolean {
        ...
        if (!isValid) {
            this.emit('order:error', { field, message: 'Поле пустое или заполнено некорректно' });
        }
...
prepareOrder(): void {
  const requiredFields: (keyof IUserOrderData)[] = ['address', 'email', 'phone', 'payment'];
  ...
  if (!Array.isArray(products) || products.length === 0) {
    ...
    for (const product of products) {
      const hasAllFields = Object.values(product).every(value => value !== null && value !== undefined);
      if (!hasAllFields) {
