...
- Метод `prepareOrder` подготавливает данные для дальнейшей обработки на сервере, генерирует событие `order:ready`, если поля валидны.
